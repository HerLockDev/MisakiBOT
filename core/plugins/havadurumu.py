...
async def havadurumu(client:Client, message:Message):
    sehır = message.command[1]
    site_api = "6fded1e1c5ef3f394283e3013a597879"
    main_url = "https://api.openweathermap.org/data/2.5/weather?"
    url = main_url + "q=" + sehır + "&appid=" + site_api
    ham_veri = requests.get(url)
    await message.edit("⚒Thor UserBot bilgileri arıyor... 🔍.")
    await asyncio.sleep(1.25)
    veri_json = ham_veri.json()
    
    if veri_json["cod"] == "404":
        await message.edit("{} isimli bir şehir bulunmamakta.".format(sehır))
        await asyncio.sleep(1.25)
    else:
    ...
    await message.edit("🔆{} şehrinin:\n🌡Sıcaklığı: {}\n🌇Hissedilen sıcaklığı: {}\n🌳Gökyüzünün durumu: {}\n🗺Rüzgar hızı: {}\nŞehrin bulunduğu ülke: {}".format(sehır,csıcaklık,chis,acıklama,wind_speed,country))
    await asyncio.sleep(1.25)
