...
async def  botyap(client:Client, message:Message):
    ...
    try:
        ...
        return
    if cmd1 == "" or cmd2 == "":
        await client.send_message("Bilgileri eksik girdiniz tekrar deneyin.",chat_id=message.chat.id)
    try:
...
You come to me asking for more than 20 bots. But you don't ask with respect. You don't offer friendship. You don't even think to call me Botfather.""":
            await client.send_message("Çok fazla botunuz olduğu için bot silmeniz gerekiyor.",chat_id=message.chat.id)
        ...
        if mesaj == "Sorry, the username must end in 'bot'. E.g. 'Tetris_bot' or 'Tetrisbot'":
            await client.send_message("Username bot ile bitmeil örneğin ThorUserBot",chat_id=message.chat.id)
...
You come to me asking for more than 20 bots. But you don't ask with respect. You don't offer friendship. You don't even think to call me Botfather.""":
        ...
        if mesaj == "Sorry, this username is invalid.":
            await client.send_message("Username bot ile bitmeli örneğin ThorUserBot",chat_id=message.chat.id)
        if mesaj == "Sorry, this username is already taken. Please try something different.":
            await client.send_message("Bu username daha önce alınmış lütfen başka bir tane deneyin.",chat_id=message.chat.id)
        await client.send_message("İşlem bitti:\nBotunuzun ismi:{cmd1}\nBotunuzun usernamesi:{cmd2}")
CmdHelp("botyap").add_command("byap", "<bot ismi> <bot usernamesi>", "@BotFather ile yeni bot oluşturur.Bot usernamesi bot ile bitmeli.", "byap thor thoruserbot").add()